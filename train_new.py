...
def valid(args, model, writer, test_loader, epoch, global_step):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        x, y = batch
        with torch.no_grad():
            logits, heads_select_rate = model(x)
    ...
    return val_accuracy, heads_select_rate
...
def train(args, model):
    ...
    for epoch in range(args.num_epochs):
        ...
        with torch.no_grad():
            val_accuracy, heads_select_rate = valid(args, model, writer, test_loader, epoch, global_step)
        ...
        with open(result_file, 'a') as file:
            ...
            file.write('\n')
            file.write("heads_select_rate in epoch: %s" % str(heads_select_rate))
            file.write('\n')
